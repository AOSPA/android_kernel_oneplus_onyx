...
#define DEFAULT_MAX_CORES_SCREENOFF	2
...
static struct asmp_param_struct {
	...
	unsigned int min_boost_freq;
 	unsigned int max_cores_screenoff;
	bool enabled;
	u64 boost_lock_dur;
} asmp_param = {
	...
	.max_cores_screenoff = DEFAULT_MAX_CORES_SCREENOFF,
};
...
static void asmp_lcd_suspend(struct work_struct *work)
{
	...
	for_each_online_cpu(cpu)
		if (cpu && num_online_cpus() > asmp_param.max_cores_screenoff)
			cpu_down(cpu);
}
...
static __ref int set_max_cores_screenoff(const char *val, const struct kernel_param *kp)
{
	...
	if (ret)
		return -EINVAL;
	if (i < 1 || i > max_cpus || i > num_possible_cpus())
		return -EINVAL;
	if (i > max_cpus)
		asmp_param.max_cores_screenoff = max_cpus;
